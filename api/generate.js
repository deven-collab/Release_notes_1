...
- Each entry must follow this exact structure:
  1. "title": A clear, benefit-led headline (e.g. "Clearer Status Indicators Across Campaign Views")
  2. "summary": 2-3 sentences explaining what changed and why it benefits the user
  3. "bullets": An array of 2-4 concise bullet points highlighting specific changes. Each bullet starts with a strong verb and focuses on user benefit.
...
- You MUST produce one output entry for every single ticket provided. Never skip, drop, or omit any ticket.
- Only merge two tickets if they have literally identical titles and descriptions. Otherwise keep them separate.
- If you cannot determine the correct category, default to "Improvements".
- If a ticket is engineering-only with no product context, place it under "Platform & Performance" with generic benefit-led language and 1-2 generic bullets.
- If a ticket description is too vague to write a meaningful note, return: { "flag": true, "key": "...", "reason": "..." }
- The number of items in your output array MUST equal the number of tickets provided.
...
- Engineering-only tickets → "Platform & Performance" with generic language only. Example: "Ongoing platform improvements have been applied to ensure a consistent and stable experience."`;
...
Return ONLY a valid JSON array of exactly ${ticketCount} items. No preamble, no markdown fences, no extra text. Each item:
{
  "key": "...",
  "ppKey": "...",
...
The "client" field: use the client name from the ticket data.
The "ppKey" field: use the ticket key as provided (e.g. PP-1225).`;
